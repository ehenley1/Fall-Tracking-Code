...
class IgnoreCircularReferenceEncoder(json.JSONEncoder):
...
def main():
    ...
    parser.add_argument(
        '--tracking-thr', type=float, default=0.5, help='Tracking threshold')
    parser.add_argument(
        '--euro',
        action='store_true',
    ...
    parser.add_argument(
        ...
        type = str,
        default = '',
        help = 'Output path for the keypoints JSON file. Default is not saving the keypoints.')
    ...
    if args.out_json:
        ...
        def remove_circular_references(obj, visited=None):
            if visited is None:
                visited = set()
            ...
            if isinstance(obj, list):
                return [remove_circular_references(item, visited) for item in obj]
            elif isinstance(obj, dict):
                return {k: remove_circular_references(v, visited) for k, v in obj.items()}
            elif isinstance(obj, np.ndarray):
                return obj.tolist()
            else:
                return obj
        all_pose_results_cleaned = remove_circular_references(all_pose_results)

        with open(args.out_json, 'w') as f:
            json.dump(all_pose_results_cleaned, f, cls=NumpyArrayEncoder)
